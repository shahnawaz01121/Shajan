...
module.exports.run = async({api,event,args}) => {
  ...
  {body:`———»ADMIN BOT«———\n❯ Name: 𝕊𝕙𝕒𝕙𝕟𝕒𝕨𝕒𝕫 𝕤𝕚𝕕𝕕 🖤\n❯ Facebook: https://https://m.facebook.com/ammi ka.ladla.official\n❯ Thanks for using ${global.config.BOTNAME} bot`,
    attachment: fs.createReadStream(__dirname + "/cache/333.jpg")}, event.threadID, () => 
    fs.unlinkSync(__dirname + "/cache/333"));  
      return request(encodeURI(`https://www.facebook.com/share/19RdPELWiE/`)).pipe(
fs.createWriteStream(__dirname+'/cache/333.jpg')).on('close',() => callback());
...
if (args[0] == "user") { 
    ...
    var gender = sex == 2 ? "Male" : sex == 1 ? "Female" : "Tran Duc Bo";
    var callback = () => api.sendMessage({body:`Name: ${name}` + `\nPersonal URL: ${url}` + `\n💦User name: ${sn}\nUID: ${mentions}\nSex: ${gender}\nMake friends with bots: ${b}`,attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/333.jpg"),event.messageID);   
